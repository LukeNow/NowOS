...
static void heap_insert_header(uint32_t addr, node_header_t *header)
{
	memcpy((void *)addr, header, sizeof(node_header_t));
}
...
static node_header_t *heap_find_free_node(uint32_t alloc_size)
{
	...
	for (int i = 0; i < list_len; i++) {
		...
		if (node->size >= alloc_size && node->size < best_node->size)
			best_node = node;
	}
...
static void heap_merge_left(node_header_t *node)
{
	...
	/* Remove these two free nodes from the list */
	...
	/* Add our new free node */
	insert_list((void *)new_node, &free_list);
}

static void heap_merge_right(node_header_t *node)
{
...
static void heap_split_node(node_header_t *free_node, uint32_t new_alloc_size)
{
	...
	heap_set_header((uint32_t)new_used_node, USED, NULL, NULL, new_alloc_size);
	
	/* This new free node gets the rest of the space */
	new_free_node = (node_header_t *)(free_node + total_new_alloc_size);
	
	uint32_t new_free_alloc_size = total_size_left - sizeof(node_header_t);
	if (new_free_alloc_size <= 0) {
		WARN("NEW ALLOC IS LESS THAN 0");
	}
	
	heap_set_header((uint32_t)new_free_node, FREE, NULL, NULL, 
			new_free_alloc_size);
	
	/* Set new used node ptrs */
	new_used_node->prev_ptr = old_prev_ptr;
	new_used_node->next_ptr = new_free_node;
	/* Set new free node ptrs */
	new_free_node->prev_ptr = new_used_node;
	new_free_node->next_ptr = old_next_ptr;
...
//TODO We need to align the data
void *kmalloc(size_t size)
{
	...
	if (!node) {
		//TODO Expand heap
		PANIC("kmalloc: WE DID NOT FIND FREE NODE");
		return NULL;
	}
	...
	return (void *)((uint32_t)node + sizeof(node_header_t));
}
...
void kfree(void *ptr)
{
	...
	/* Add it to free list */
	//insert_list((void *)node, &free_list);
